...
export default function LeadDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (data?.lead) {
      document.title = `${data.lead.companyName} | Lead Details | TattvaLogic`;
    }
  }, [data]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-2 text-sm">
        <Link href="/sales-dashboard/crm" className="font-bold text-zinc-500 hover:text-zinc-900 transition-colors flex items-center gap-1">
          <ArrowLeft size={16} /> Pipeline
        </Link>
        <span className="text-zinc-300">/</span>
        <span className="font-bold text-zinc-900">{lead.companyName}</span>
      </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white p-6 rounded-3xl border border-zinc-200 shadow-sm">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h2 className="text-2xl font-black text-zinc-900 leading-tight">{lead.companyName}</h2>
                <p className="text-sm font-bold text-primary mt-1">{lead.serviceInterest}</p>
                <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-widest mt-2">ID: {lead.leadId}</p>
              </div>
